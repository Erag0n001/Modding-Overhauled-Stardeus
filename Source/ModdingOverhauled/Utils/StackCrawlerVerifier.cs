...
public partial class StackCrawler {
    private static readonly ThreadLocal<Queue<StackState>> StatesToRead = new();
    /// <summary>
    ...
    {
        Exception ex = null;
        var stackTypes = new List<Type>();
        var allLabels = new Dictionary<Label, LabelStatus>();
        var states = StatesToRead.Value = new Queue<StackState>();
        states.Enqueue(new StackState([], 0));
        try {
            for (var i = 0; i < All.Count; i++) {
                ...
                if (c.opcode == OpCodes.Ret) {
                    ...
                    if (returnType == typeof(void)) {
                        if (stackTypes.Count > 0) {
                            throw new Exception(
                                $"Group {GetGroupAt(i)} at code {All[i]} ({i}) tried to return {returnType} on a void method");
                        }
                    }
                    else {
                        if (stackTypes.Count > 1) {
                            throw new Exception(
                                $"Group {GetGroupAt(i)} at code {All[i]} ({i}) tried to return {string.Join(",", stackTypes)} types");
                        }

                        if (stackTypes.Last() != returnType) {
                            throw new Exception(
                                $"Group {GetGroupAt(i)} at code {All[i]} ({i}) tried to return {stackTypes.Last()} on a {returnType} method");
                        }
                    }
                ...
                if (HandleConditionalBr(c, out ex, stackTypes, allLabels)) {
                    if (ex != null) {
                        throw new Exception($"Group {GetGroupAt(i)} at code {All[i]} ({i}) returned {ex}");
                    }
            ...
            foreach (var l in allLabels.Values) {
                if (!l.IsAssigned) {
        ...
        bool HandleLabel(CodeInstruction c, out Exception exception)
        {
            ...
            if (labels != null && labels.Count > 0)
            {
                ...
                {
                    if (!allLabels.TryGetValue(l, out var status))
                    {
        ...
        {
            exception = null;
            var flag = OpCodes.Br == c.opcode || OpCodes.Br_S == c.opcode;
            if (UnConditionalBrOpCodes.Contains(c.opcode) && !flag)
            {
                var t = stackTypes.Last();
                if (!(t.IsPrimitive || t.IsValueType || t.IsClass))
                {
                    exception = new  Exception($"Tried to have a conditional check, but the last type {stackTypes.Last()} which is not valid. Expected class, null, integer or boolean");
                    return true;
                }
                stackTypes.Remove(stackTypes.Last());
                flag = true;
            }
            else if (LeaveLabels.Contains(c.opcode)) {
                ...
                if (!allLabels.TryGetValue(label, out var state))
                {
            ...
            if (flag)
            {
                ...
                if (!allLabels.TryGetValue(label, out var labelStatus))
                {
                    labelStatus = new LabelStatus();
                }

                labelStatus.HasProperJump = true;

                var currentStack = stackTypes;
 
                
                allLabels[label] = labelStatus;
            }
    ...
    private Exception HandleTypesFromMethod(CodeInstruction c, List<Type> stack)
    {
        ...
        var expectedCount = paramTypes.Length + (method.IsStatic ? 0 : 1);
    ...
    private class StackState(List<Type> stackTypes, int start) {
        public readonly List<Type> Types = stackTypes;
        public readonly int StartIndex = start;
    }
    ...
    private class LabelStatus
    {
        ...
        public Type[] StackOnArrival;
        public LabelStatus()
        {
        }

        public bool SetOrCompareStackOnArrival(Type[] currentStack, out Exception exception) {
            if (StackOnArrival == null) {
                StackOnArrival = currentStack.ToArray();
                exception = null;
                return true;
            }

            if (!StackOnArrival.SequenceEqual(currentStack)) {
                exception = new Exception($"Label has different stack state." +
                                          $"\nExpected {string.Join(", ", StackOnArrival.Select(x => x.FullName))}" +
                                          $"\nGot {string.Join(", ", currentStack.Select(x => x.FullName))}");
                return false;
            }
            exception = null;
            return true;
